...
void ArcLengthSpline::setData(const Eigen::VectorXd &X_in,const Eigen::VectorXd &Y_in,const Eigen::VectorXd &Z_in)
{
    // set input data if x, y and z have same length
    // compute arc length based on an piecewise linear approximation
    if(X_in.size() == Y_in.size() && X_in.size() == Z_in.size() && Y_in.size() == Z_in.size()){
        path_data_.X = X_in;
        path_data_.Y = Y_in;
        path_data_.Z = Z_in;
...
void ArcLengthSpline::setRegularData(const Eigen::VectorXd &X_in,const Eigen::VectorXd &Y_in,const Eigen::VectorXd &Z_in,const Eigen::VectorXd &s_in) {
...
PathData ArcLengthSpline::resamplePath(const CubicSpline &initial_spline_x,const CubicSpline &initial_spline_y,const CubicSpline &initial_spline_z,const double total_arc_length) const
{
...
RawPath ArcLengthSpline::outlierRemoval(const Eigen::VectorXd &X_original,const Eigen::VectorXd &Y_original,const Eigen::VectorXd &Z_original) const
{
    ...
    if (X_original.size() != Y_original.size() || X_original.size() != Z_original.size() || Y_original.size() != Z_original.size()){
        //error
    }
//    std::cout << X_original << std::endl;
    ...
    for(int i=1;i<n_points-1;i++){
        ...
        // if this distance is larger than 0.7 the mean distance add this point to the new X-Y-Z path
...
void ArcLengthSpline::fitSpline(const Eigen::VectorXd &X,const Eigen::VectorXd &Y,const Eigen::VectorXd &Z)
{
...
//    std::cout << s_approximation << std::endl;
    ...
    // 1. re-sample
    first_refined_path = resamplePath(first_spline_x,first_spline_y,first_spline_z,total_arc_length);
    ...
    // 2. re-sample
    second_refined_path = resamplePath(second_spline_x,second_spline_y,second_spline_z,total_arc_length);
    ////////////////////////////////////////////
    setRegularData(second_refined_path.X,second_refined_path.Y,second_refined_path.Z,second_refined_path.s);
...
void ArcLengthSpline::gen3DSpline(const Eigen::VectorXd &X,const Eigen::VectorXd &Y,const Eigen::VectorXd &Z)
{
    ...
    RawPath clean_path;
    clean_path = outlierRemoval(X,Y,Z);
    // successively fit spline and re-sample
    fitSpline(clean_path.X,clean_path.Y,clean_path.Z);
