...
//    std::cout << x.size() << std::endl;
    ...
    EXPECT_TRUE((yt-ytt).sum()/NV <= 1e-4 && (dyt-dytt).sum()/NV <= 1e-3 && (ddyt-ddytt).sum()/NV <= 1e-1);
}
...
TEST(TestSpline, TestArcLengthSpline)
{
    ...
    // test 3-D arc length spline approach
    // given a circle with randomly distributed points

    mpcc::ArcLengthSpline threeDspline = mpcc::ArcLengthSpline(json_paths,robot);
    ...
    threeDspline.gen3DSpline(X,Y,Z);
    ...
    for(int i=0;i<NV;i++){
        pos = threeDspline.getPostion(phiv(i));
